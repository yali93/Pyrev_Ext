...
from System.Windows.Forms import Form, TextBox, Label, Button, MessageBox, CheckBox, ComboBox, ComboBoxStyle, RadioButton
...
printsetting = [i for i in printSettingCollector]
SHEETList=[]
mechlist=[]
infolist=[]
eleclist=[]
...
for sheet in sheets:
    ...
    sheetGroupProperty = sheet.LookupParameter("MRT_BRO_Sheet Grouping").AsString()
    
    if sheetGroupProperty is None:
    	misclist.append(sheet)
    
    elif sheetGroupProperty.ToLower().ToString().Contains('mechanical'):
        mechlist.append(sheet)
    elif sheetGroupProperty.ToLower().ToString().Contains('electrical'):
        eleclist.append(sheet)
    elif sheetGroupProperty.ToLower().ToString().Contains('info'):
        infolist.append(sheet)
...
class MyClass(Form):
	...
	def __init__(self):
		...
		self.label = Label()
		self.label.Text = ""
		self.label.Location = Point(650, 300)
		...
		#____________________dROP DOWON LIST____________
		self.dropdown = ComboBox()
		printsettinglist = [i.Name for i in printSettingCollector]
		printsettinglist.insert(0,'Print Setting Profile')
		arrayprintset = Array[str](printsettinglist)
		ComboBox.Items.PropertyType.AddRange(self.dropdown.Items,arrayprintset)
		self.dropdown.Location = Point(50,800)
		self.dropdown.Width = 400
		self.dropdown.Text = "Print Settings Profiles"
		self.dropdown.Name = "Settings"
		...
		self.CheckBox1.Location = Point(600,100)
		...
		#_____________ Radio Button  _____________________________
		self.CheckBox2 = RadioButton()
		self.CheckBox2.Location = Point(600,140)
		self.CheckBox2.AutoSize = True
		self.CheckBox2.Width = 100
		self.CheckBox2.CheckedChanged += self.HideCheck
		self.CheckBox2.Text = "Print All \n Mechanical Sheets"
		self.Controls.Add(self.CheckBox2)

        		#_____________ RadioButton  2_____________________________
		self.CheckBox3 = RadioButton()
		self.CheckBox3.Location = Point(600,180)
		self.CheckBox3.AutoSize = True
		self.CheckBox3.Width = 100
		self.CheckBox3.CheckedChanged += self.HideCheck
		self.CheckBox3.Text = "Print all \n Electrical Sheets"
		self.Controls.Add(self.CheckBox3)
    	
        #_________ Radio Button 3_____________________
		self.CheckBox4 = RadioButton()
		self.CheckBox4.Location = Point(600,220)
		self.CheckBox4.AutoSize = True
		self.CheckBox4.Width = 100
		self.CheckBox4.CheckedChanged += self.HideCheck
		self.CheckBox4.Text = "Custome"
	...
	def HideCheck(self,sender,args):
		
		if self.CheckBox3.Checked:
			self.listView1.CheckBoxes = False
	...
	def OnClick(self,sender,args):
		...
		if self.CheckBox1.Checked:
			checkedPdf = True
			#self.CheckBox2.CheckedChanged = True
		if self.CheckBox2.Checked:
			mechprint = True
			self.CheckBox3.CheckState = False
		elif self.CheckBox3.Checked:
			elecprint = True
			self.CheckBox2.CheckState = False

		elif self.CheckBox4.Checked:
			mechprint = False
			elecprint = False
...
if mechprint == True and elecprint == False:
    sheetlistPrint = mechlist
elif elecprint == True and mechprint == False: 
    sheetlistPrint = eleclist
else:
    sheetlistPrint=sheetlist
...
for selectsheet in sheetlistPrint: # iterate through the selected sheets for printing 
	selectedSheetSize = str(sheetSize(selectsheet)) #extracts the sheet size from the selected sheet
	...
	if checkedPdf:
		...
		#that will be created 
			...
			#printSetup.SaveAs("print profile")
			#
			# printSetup.CurrentPrintSetting = PrintSetting("print profile")
		...
		MessageBox.Show("Stop Messing about","Error")
